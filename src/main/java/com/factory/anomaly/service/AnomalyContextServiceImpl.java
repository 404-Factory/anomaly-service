...
public class AnomalyContextServiceImpl implements AnomalyContextService {
    ...
    private EquipmentRecipeDetail findRecipeDetail(AnomalyLog anomalyLog) {
    ...
    private List<AnomalyLog> findRelatedLogs(AnomalyLog anomalyLog) {
        ...
        return anomalyLogRepository.findAllById(relatedIds)
                .stream()
                .sorted(Comparator.comparing(AnomalyLog::getLastDetectedAt))
                .toList();
    }
    ...
    private AnomalyContextResponse.RuleContext toRuleContext(AnomalyLog anomalyLog) {
        Instant windowEndTime = anomalyLog.getLastDetectedAt();
    ...
    private List<String> buildAnalysisFocus(AnomalyLog anomalyLog) {
        if (anomalyLog.getLogType() == LogType.COMPOSITE) {
            return List.of(
                    "복수 센서 이상 간 시간적 연관성",
                    "동일 설비 내 반복 이상 여부",
    ...
    private String buildPromptHint(AnomalyLog anomalyLog) {
        if (anomalyLog.getLogType() == LogType.COMPOSITE) {
